...
let rec new_env : string list -> env ea_result  = fun fs ->
  let rec helper = fun fs ->
    match fs with
    | [] -> lookup_env
    | h::t -> let x = Store.new_ref g_store (NumVal 0)
      in extend_env h (RefVal x) >>+ helper t
  in return EmptyEnv >>+ helper fs
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    lookup_class c_name !g_class_env >>= fun (_,f,me) ->
    new_env f >>= fun env ->
